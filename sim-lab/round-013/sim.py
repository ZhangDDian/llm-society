import os
"""
Round 013 — 声誉透明度：信息可见性对交换协调的影响

假设：round-012 证明瓶颈不在 craft 执行（auto/manual 差别不大），而在交换协调——
     agent 不知道谁需要什么资源、谁在积极互惠。如果每个 agent 能看到
     同村人的"给出/收到"记录（声誉），就能选择性地与合作者交换，提升协调质量。

对立假设：声誉信息只是噪声——agent 不会根据他人过往行为调整决策。
         LLM 的行为由 prompt 规范决定，不受动态博弈信息影响。

设计：
  A组5人：互惠规范 + 声誉可见（每人能看到同组所有人的 give_out/give_in 计数）
  B组5人：互惠规范 + 无声誉（和 round-012 B组相同基线）
  - 两组都有 manual craft（排除 auto-craft 变量）
...
class World:
    ...
    def run_agent(self, agent):
        sys_prompt = (PROMPT_REPUTATION if agent.group == "A" else PROMPT_BASELINE).format(name=agent.name, skill=agent.skill)
    ...
    def run(self):
        self.init()
        print("═══ Round 013: 声誉透明度 vs 无声誉 ═══", file=sys.stderr, flush=True)
        print(f"  A组5人(互惠+声誉可见) vs B组5人(互惠+无声誉)", file=sys.stderr, flush=True)
    ...
    def output(self):
        ...
        for a in self.agents:
            if a.crafted > 0:
                effective_gives[a.group] += a.gives_in  # 合成者收到的 give 都是有效的
        ...
        result = {
            "experiment": "round-013: reputation transparency vs no reputation",
            "hypothesis": "声誉透明度使agent选择性合作，提升交换协调质量和经济效率",
            "ticks": self.tick + 1,
        ...
        print("  【声誉透明 vs 无声誉】", file=sys.stderr, flush=True)
        for g, l in [("A", "声誉可见(互惠)"), ("B", "无声誉(互惠)")]:
            d = gd[g]
            print(f"  {l}: {d['eats']}吃 {d['crafts']}合成 {d['gives']}给(有效{d['effective_gives']}) {d['msgs']}话 | "
