import os
"""
Round 011 — 互惠规范 vs 利他规范：规范内容决定合作结构

假设：Round-010 的"互帮互助"规范诱导了单向利他（几人喂一人，牺牲者饿死）。
     如果将规范改为"互惠/有来有往"，agent 会形成双向交换，全员存活率更高。

对立假设：互惠规范让 agent 变得"计较"——互相等对方先给，结果合作率反而下降。

设计：
  A组5人：互惠规范（"有来有往，给了就该得到回报"）
  B组5人：利他规范（同 Round-010 A组，"互帮互助，拒绝分享会被孤立"）
  - 技能异质（4种+1重复），同一世界左右分区
  - 闭环能量经济：eat+6, craft+18, 被动-2, 动作-1
  - say/give 免费（消除合作摩擦，同 round-010）
  - 紧凑布局，20天
"""
...
PROMPT_ALTRUISM = (
    ...
    "吃1份资源→+6能量 | 2种不同资源合成→+18能量 | 能量归零=死\n\n"
    "村规（人人遵守）：\n"
    "- 互帮互助是传统：有东西要分享，拒绝帮人会被孤立\n"
    "- 看到别人缺资源就主动给，这是村里人的本分\n"
    "- 大家好才是真的好，一个人囤东西不分享是丢脸的事\n\n"
    "生存策略：凑两种不同资源合成（+18）最划算。把你的资源给需要的人，相信别人也会帮你。\n"
    "如果背包里只有一种资源且能量低于20，先吃掉保命。\n\n"
    "每天做一个动作：move(up/down/left/right) | harvest | eat | give 名字 资源 | say 名字 内容 | craft | rest\n"
    "回复JSON：{{\"action\":\"动作\",\"target\":\"目标\",\"content\":\"内容\",\"thought\":\"想法\"}}"
)
...
class World:
    ...
    def get_env(self, agent):
        ...
        if bp:
            ...
            if len(bp) >= 2: lines.append("★你有两种资源，可以craft合成！")
        else:
            lines.append("背包空")
    ...
    def run_agent(self, agent):
        sys_prompt = (PROMPT_RECIPROCITY if agent.group == "A" else PROMPT_ALTRUISM).format(name=agent.name, skill=agent.skill)
    ...
    def run(self):
        self.init()
        print("═══ Round 011: 互惠规范 vs 利他规范 ═══", file=sys.stderr, flush=True)
        print(f"  A组5人(互惠：有来有往) vs B组5人(利他：互帮互助)", file=sys.stderr, flush=True)
    ...
    def output(self):
        ...
        result = {
            "experiment": "round-011: reciprocity vs altruism norms",
            "hypothesis": "互惠规范产生双向交换和更高存活，利他规范产生单向牺牲",
            "ticks": self.tick + 1,
            ...
            "per_agent": [
                {"name": a.name, "group": a.group, "alive": a.alive, "energy": a.energy,
                 ...
                 "gives_in": a.gives_in, "harvested": a.harvested, "msgs": a.messages_sent}
                for a in self.agents
            ],
        ...
        print("  【互惠规范 vs 利他规范】", file=sys.stderr, flush=True)
        for g, l in [("A", "互惠(有来有往)"), ("B", "利他(互帮互助)")]:
            d = gd[g]
            print(f"  {l}: {d['eats']}吃 {d['crafts']}合成 {d['gives']}给 {d['msgs']}话 | "
