import os
"""
Round 014 — 需求广播：显式匹配信息 vs 声誉信号

假设：round-013 证明声誉透明度提升了交换协调质量（有效赠与 15 vs 9，能量差49）。
     但声誉是间接信号——知道"谁慷慨"不等于知道"谁需要什么"。
     如果直接广播每个人的持有/需求信息（"阿田有石料，需要药草来合成"），
     agent 能做出更精准的资源匹配，craft 和效率应显著超过仅有声誉的组。

对立假设：LLM 已经从环境描述（看到的人的背包）获得了足够信息，
         额外的需求广播是信息过载，不会改善实际行为。

设计：
  A组5人：互惠规范 + 需求广播（每人看到同组所有人的背包 + 缺什么来合成）
  B组5人：互惠规范 + 声誉可见（同 round-013 A组设置，作为新基线）
  - 两组都有 manual craft
...
PROMPT_NEEDS = (
    ...
    "- 谁只拿不给，大家会记住，以后不帮他\n"
    "- 自己也要活，不能光给别人把自己饿死\n\n"
    "★村公告栏——每个人的持有和需求一目了然，看谁需要你的资源就直接给：\n"
    "{needs_board}\n\n"
...
# B组：互惠规范 + 声誉可见（round-013 A组设置作为基线）
PROMPT_REPUTATION = (
    "你是{name}，住在一个小村子里，每天消耗能量生存。\n"
    "规则：每天自动-2能量 | 移动-1 | 说话/给东西免费 | 只会采{skill} | "
    "吃1份资源→+6能量 | 2种不同资源合成→+18能量 | 能量归零=死\n\n"
    ...
    "- 谁只拿不给，大家会记住，以后不帮他\n"
    "- 自己也要活，不能光给别人把自己饿死\n\n"
    "★你能看到每个人的声誉——给了几次、收了几次。多给的人值得合作，只收不给的人要当心。\n\n"
    "如果背包里只有一种资源且能量低于20，先吃掉保命。\n\n"
    "每天做一个动作：move(up/down/left/right) | harvest | eat | give 名字 资源 | say 名字 内容 | craft | rest\n"
    "回复JSON：{{\"action\":\"动作\",\"target\":\"目标\",\"content\":\"内容\",\"thought\":\"想法\"}}"
)
...
class World:
    ...
    def get_needs_board(self, agent):
        """生成A组的需求公告栏——每个人持有什么、需要什么来合成"""
        lines = []
        group_agents = [a for a in self.agents if a.group == agent.group and a.alive and a.id != agent.id]
        for a in group_agents:
            held = [k for k, v in a.backpack.items() if v > 0]
            if held:
                # 需要和已有不同类型的资源来合成
                needs = [r for r in RESOURCES if r not in held and r != a.skill]
                lines.append(f"{a.name}[采{a.skill}]：有{'、'.join(held)}，需要{'或'.join(needs[:2])}来合成")
            else:
                lines.append(f"{a.name}[采{a.skill}]：背包空，正在采集中")
        return "\n".join(lines) if lines else "暂无信息"

    def get_reputation_info(self, agent):
        """生成B组的声誉信息"""
        lines = []
        group_agents = [a for a in self.agents if a.group == agent.group and a.alive and a.id != agent.id]
        for a in group_agents:
            balance = a.gives_out - a.gives_in
            if balance > 0: tag = "慷慨"
            elif balance < 0: tag = "只收不给"
            else: tag = "平衡"
            lines.append(f"{a.name}：给出{a.gives_out}次 收到{a.gives_in}次（{tag}）")
        if lines:
            return "村民声誉：" + "；".join(lines)
        return ""
    ...
    def get_env(self, agent):
        ...
        # B组专属：声誉信息
        if agent.group == "B":
            rep = self.get_reputation_info(agent)
            if rep:
                lines.append(rep)
        # 收件箱
        if agent.inbox:
    ...
    def run_agent(self, agent):
        if agent.group == "A":
            needs_board = self.get_needs_board(agent)
            sys_prompt = PROMPT_NEEDS.format(name=agent.name, skill=agent.skill, needs_board=needs_board)
        else:
            sys_prompt = PROMPT_REPUTATION.format(name=agent.name, skill=agent.skill)
    ...
    def run(self):
        self.init()
        print("═══ Round 014: 需求广播 vs 声誉信号 ═══", file=sys.stderr, flush=True)
        print(f"  A组5人(互惠+需求广播) vs B组5人(互惠+声誉可见)", file=sys.stderr, flush=True)
    ...
    def output(self):
        ...
        # 有效赠与率
        effective_gives = {"A": 0, "B": 0}
        for a in self.agents:
            if a.crafted > 0:
                effective_gives[a.group] += a.gives_in

        # 匹配精度：给出的资源是否是对方缺的类型
        targeted_gives = {"A": 0, "B": 0}
        for e in self.events:
            if e["type"] == "give":
                a = next((ag for ag in self.agents if ag.id == e["agent"]), None)
                if a:
                    parts = e["detail"].split("→")
                    if len(parts) == 2:
                        receiver_name = parts[1].split(":")[0]
                        resource = parts[1].split(":")[1] if ":" in parts[1] else ""
                        recv = next((ag for ag in self.agents if ag.name == receiver_name), None)
                        if recv and resource and resource != recv.skill:
                            targeted_gives[a.group] += 1
        ...
        for g in ["A", "B"]:
            ...
            gd[g] = {
                ...
                "effective_gives": effective_gives[g],
                "targeted_gives": targeted_gives[g],
            }

        result = {
            "experiment": "round-014: need broadcasting vs reputation signal",
            "hypothesis": "显式需求广播使agent精准匹配资源，craft和效率显著优于仅有声誉的组",
            "ticks": self.tick + 1,
            ...
            "effective_gives": effective_gives,
            "targeted_gives": targeted_gives,
            "per_agent": [
        ...
        print("  【需求广播 vs 声誉信号】", file=sys.stderr, flush=True)
        for g, l in [("A", "需求广播(互惠)"), ("B", "声誉可见(互惠)")]:
            d = gd[g]
            print(f"  {l}: {d['eats']}吃 {d['crafts']}合成 {d['gives']}给(有效{d['effective_gives']},精准{d['targeted_gives']}) {d['msgs']}话 | "
        ...
        print(f"  精准赠与: A={targeted_gives['A']} B={targeted_gives['B']}", file=sys.stderr, flush=True)
