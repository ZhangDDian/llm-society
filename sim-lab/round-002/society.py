...
class Society:
        ...
        try:
            resp = httpx.post(
                ...
                json={
                    "model": MODEL,
                    "messages": [
                        {"role": "system", "content": f"你是{agent.name}，一个有情绪、有好奇心的人。你能感到饿、无聊、恐惧、亲近。活着比什么都重要，但活着不是唯一的事。用JSON回复你的动作，不要解释。"},
    ...
    def run(self):
        self.init_world()
        print(f"═══ Round 002: 精确感知 + 人格prompt ═══", flush=True)
        print(f"  变量：system prompt 从'游戏角色'改为'有情绪的人'", flush=True)
        print(f"  感知：与 000b 一致（方向+步数+能量数字+生存提示）", flush=True)
